...
class Platoon(models.Model):
    ...
    base_code = models.CharField(max_length=4)
...
def Claim_Base(code):
    ...
    '''
    try:
        capturers = Section.objects.get(ambush_code=code[0:3])
        captees = Platoon.objects.get(base_code=code[3:7])
    except:
        return "Base Code Doesn't Exist"

    a = Action(
        section = capturers,
        action = "Captured " + str(captees.number) + " Platoon Base ",
        points = 300,
        )
    a.save()
